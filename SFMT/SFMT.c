...
//My own function added to get the same functionality as sfmt_fill_array64, but without the large block size requirement
void sfmt_fill_small_array64(sfmt_t * sfmt, uint64_t *array, unsigned int size)
{
	unsigned int i;
	for(i = 0; i < size; i++)
	{
		array[i] = sfmt_genrand_uint64(sfmt);
	}
}
